...
metrics:
  ...
  - name: MOS
    value: 4.00
    system: EmoSteer-TTS + F5-TTS (EI-MOS interpolation)
    testset: in-distribution
  - name: MOS
    value: 4.02
...
related_concepts: [emotion-synthesis, flow-matching, zero-shot-tts, prosody-control]
...
> **Xie et al.** (HKUST (Guangzhou) / Tencent AI Lab) · [→ Paper](https://arxiv.org/abs/2508.03543) · Demo: ✓ · Code: ?
>
...
![Overview of EmoSteer-TTS. Steering vectors and steering weights are derived from pairs of neutral and emotional reference speech. During inference, these vectors are used to modulate the activations in a TTS model, guiding it to synthesize speech that reflects the desired emotion.](assets/2508.03543/figure-3.png)
...
EmoSteer-TTS applies to [[2025.acl-long.313|F5-TTS]], E2-TTS, and CosyVoice2 ([[2412.10117|CosyVoice 2]]). The difference-in-means steering vector construction shares principles with [[2508.02038|Marco-Voice]], which uses the same technique for constructing emotion embeddings. The flow-matching DiT architecture connects to [[flow-matching]]. The emotion control capability connects to [[emotion-synthesis]] and [[prosody-control]] concept pages. The in-corpus reference [[2412.06602|Controllable TTS Survey]] and [[2504.12867|EmoVoice]] provide context for the broader emotion TTS landscape. The training-free paradigm contrasts with the supervised RLHF post-training approach in [[2508.16332|Vevo2]]. Back-linked by [[2510.05758|EMORL-TTS]], which uses EmoSteer-TTS as a comparison baseline for RL-based emotion control.
