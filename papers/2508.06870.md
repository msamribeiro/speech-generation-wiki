...
related_concepts: ["multilingual-tts", "evaluation-metrics", "subjective-evaluation", "gan-vocoder"]
related_papers: []
field_significance:
  level: low
...
- Standard neural TTS pipelines (acoustic model + neural vocoder) can produce intelligible speech for previously unsupported tonal scripts with as little as 40 minutes of single-speaker training data.
- The absence of explicit tone modelling in a Tacotron 2 pipeline leads to measurable degradation in prosody quality for tonal languages, even when the training corpus is small.
- Developing a phoneme mapping between a novel script and an established phoneme set (such as ARPAbet) is a necessary and non-trivial prerequisite before standard TTS toolkits can be applied to indigenous writing systems.
...
This paper is most directly relevant to [[multilingual-tts]] and [[subjective-evaluation]]. The use of HiFi-GAN as vocoder connects to [[gan-vocoder]]. The evaluation methodology (MOS collected from native speakers, no automated metrics) is worth noting for [[evaluation-metrics]].
