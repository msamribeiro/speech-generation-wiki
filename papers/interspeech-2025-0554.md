...
RapFlow-TTS advances [[flow-matching]] by introducing consistency constraints on straight ODE trajectories, analogous to how Comospeech advanced [[diffusion-tts]] with consistency distillation. The adversarial learning component connects to [[gan-vocoder]] patterns applied at the mel-spectrogram level. The evaluation framework (MOS + WER + RTF) is documented in [[evaluation-metrics]] and [[subjective-evaluation]]. No in-corpus references found; primary comparisons are to Matcha-TTS, Grad-TTS, VoiceFlow, FastSpeech2, and Comospeech (all out-of-corpus).
