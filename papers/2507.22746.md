...
- Composing autoregressive generation across fixed-length token blocks with parallel flow-matching denoising within each block can simultaneously provide KV-cache efficiency and bidirectional contextual refinement.
- Neural codecs using finite scalar quantisation can preserve speaker similarity and intelligibility at frame rates (12.5 Hz) where STFT-based vocoders suffer significant quality degradation.
- Continuous denoising models can implicitly classify discrete token targets through appropriate embedding design, without requiring a separate discrete language model head.
- Reducing the token frame rate is a more tractable path to low-latency hybrid AR-diffusion TTS than increasing diffusion step efficiency alone, given the quadratic scaling of self-attention with sequence length.
...
- [[2504.10352]] — Pseudo-autoregressive codec language models for zero-shot TTS, related in-corpus reference.
