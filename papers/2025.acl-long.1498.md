...
The use of UTMOS, PESQ, ViSQOL, WER, and SPK-SIM as evaluation tools in a codec-to-TTS pipeline is a useful template for [[evaluation-metrics]]. The consistency accuracy metric itself is a novel contribution that deserves to be considered a standard diagnostic tool alongside existing codec benchmarks. A complementary direction — replacing RVQ entirely with continuous tokens — is explored by [[2025.findings-naacl.184]], which achieves better high-frequency information retention by removing discretization rather than improving consistency within it.
