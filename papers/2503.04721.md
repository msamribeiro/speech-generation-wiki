...
- Explicit turn-state prediction modules reduce unwanted interruptions during intra-turn speaker pauses more effectively than purely end-to-end architectures trained on dialogue data.
- End-to-end full-duplex speech models respond with lower latency but produce less semantically coherent replies when confronted with user interruptions than cascaded systems with a frozen LLM backbone.
- Backchannel generation remains a largely unsolved capability in current full-duplex spoken dialogue models, with most open-source systems producing near-zero backchannel frequency.
- Commercial full-duplex systems exhibit better-calibrated turn-taking behaviour on naturalistic data than open-source research models, suggesting that VAD design and deployment-scale tuning contribute substantially to perceived conversational naturalness.
- Automatic metrics for full-duplex dialogue can be made reproducible and model-agnostic without human raters, enabling rapid cross-system comparisons that corpus-level statistics and user studies cannot efficiently provide.
...
Concepts most directly informed by this paper: [[spoken-language-model]], [[speech-to-speech]], [[evaluation-metrics]], [[subjective-evaluation]].

Papers that cite this benchmark: [[2508.07375]] (TurnGuide evaluates full-duplex turn-taking with Full-Duplex-Bench), [[2508.17623]] (EMO-Reasoning uses Full-Duplex-Bench as a related evaluation framework), [[2509.03940]] (VoxRole cites Full-Duplex-Bench as a complementary spoken dialogue benchmark).
