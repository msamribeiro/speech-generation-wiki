...
related_concepts: [spoken-language-model, self-supervised-speech, speech-to-speech, subjective-evaluation, instruction-conditioned-tts, multilingual-tts]
related_papers: []
---

# Distilling an End-to-End Voice Assistant Without Instruction Training Data

**Paper:** [https://aclanthology.org/2025.acl-long.388](https://aclanthology.org/2025.acl-long.388)

**One-sentence contribution:** DiVA trains an end-to-end speech large language model using only ASR data and cross-modal knowledge distillation from a frozen text LLM, entirely bypassing the need for annotated speech instruction data.
...
This paper is a direct contribution to [[spoken-language-model]]: it proposes a new training paradigm (cross-modal context distillation without instruction data) for speech LLMs, addressing the capability-forgetting problem that SFT-based systems exhibit. The Whisper encoder reuse connects it to [[self-supervised-speech]], where Whisper's supervised representations are shown to encode rich audio-text alignment useful beyond ASR. The Q-Former bridge architecture and the end-to-end speech-in, text-out design are core to [[speech-to-speech]] pipelines. The user preference study methodology (side-by-side Prolific workers, 72% win rate metric) connects to [[subjective-evaluation]] as a non-MOS alternative for SCA quality assessment. The system's ability to generalize to multilingual speech translation without multilingual training data touches [[multilingual-tts]] in a SCA context. The instruction-following capability being retained from the text LLM without any instruction data connects to [[instruction-conditioned-tts]] by showing that distillation can transfer instruction following from text to speech without the annotation cost.
