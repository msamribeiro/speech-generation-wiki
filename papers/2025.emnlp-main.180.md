...
related_concepts: [instruction-conditioned-tts, prosody-control, emotion-synthesis, subjective-evaluation, self-supervised-speech]
related_papers: []
field_significance:
  level: moderate
...
- Large-scale instruction-conditioned TTS datasets with rich abstract tags produce substantially higher style consistency and naturalness than datasets limited to basic signal-extractable tags.
- Perceptual speaker similarity, rather than speaker verification similarity, is the appropriate proxy for propagating subjective intrinsic style tags across speakers.
- Combining expressivity filtering, semantic matching, and acoustic LLM scoring is necessary for reliable automatic annotation of situational style tags at scale; omitting any stage degrades quality.
- Style-expressive TTS models trained on rich paralinguistic data incur intelligibility trade-offs that reflect genuine style fidelity rather than generation quality degradation.
- The lack of automatic metrics for style consistency in instruction-conditioned TTS remains a bottleneck: current evaluation relies entirely on expensive crowd-sourced human judgement.
