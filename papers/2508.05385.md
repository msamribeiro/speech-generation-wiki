...
related_concepts: ["emotion-synthesis", "prosody-control", "evaluation-metrics", "subjective-evaluation", "self-supervised-speech", "multilingual-tts"]
...
- Automated annotation pipelines for non-verbal vocalizations can match or exceed manually-annotated datasets in downstream NV generation and understanding tasks, while scaling at substantially lower cost.
- Positional accuracy of non-verbal tag annotations is a more important determinant of NV controllability in TTS than dataset size alone.
- Frame-level NV detection models trained exclusively on one language can generalise to structurally different languages without retraining, suggesting that acoustic features of non-verbal sounds are largely language-agnostic.
- Rule-based NV data augmentation produces measurably worse NV controllability in fine-tuned TTS systems compared to models trained on naturally-occurring vocalizations.
...
Core concepts: [[emotion-synthesis]], [[prosody-control]], [[evaluation-metrics]], [[subjective-evaluation]], [[self-supervised-speech]], [[multilingual-tts]]

In-corpus papers: [[2509.14946]]
