...
metrics:
  - name: MUSHRA
    value: 76.3
    system: proposed
    testset: MUSDB18 (2.7 kbps, A/B preference)
  - name: MUSHRA
    value: 55.0
    system: proposed
    testset: MUSDB18 (5.3 kbps, A/B preference)
  - name: MUSHRA
    value: 50.8
    system: proposed
    testset: MUSDB18 (8 kbps, A/B preference)
code_available: null
demo_available: null
...
- Operating in the time-frequency domain enables neural codecs to achieve higher perceptual quality for full-band audio than equivalent waveform-domain architectures, especially at low bit rates.
- Cross-channel phase coherence in multi-channel neural codecs requires joint processing of audio channels in at least some encoder layers, and neither fully independent nor fully joint encoding is optimal.
- Multi-scale spectral discriminators are effective for training high-quality neural codecs without requiring waveform-domain discriminators.
- Biased quantizer dropout towards low codebook counts during training improves codec quality at the low bit rate end without sacrificing high bit rate performance.
- Real-time streaming neural codec inference at 48 kHz stereo is achievable on a desktop CPU with an 80 ms architectural latency when using causal convolutions and a minimal look-ahead.
...
Core concepts: [[neural-codec]] · [[gan-vocoder]] · [[streaming-tts]]
