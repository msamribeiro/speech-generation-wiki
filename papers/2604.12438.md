...
related_concepts: [streaming-tts, neural-codec, transformer-enc-dec-tts, autoregressive-codec-tts]
...
Autoregressive codec language models such as [[2301.02111|VALL-E]] eliminate the vocoder but introduce prohibitive temporal latency by flattening multi-layer RVQ codes into a single sequential prediction chain. Masked iterative models still require multiple refinement passes. A fully feed-forward path from text to discrete codec tokens — without temporal autoregression or iterative sampling — was not available in a form suitable for real-time single-speaker streaming.
...
This paper develops a streaming non-autoregressive path within [[streaming-tts]] that bypasses the vocoder bottleneck by operating directly in the [[neural-codec]] latent space. The backbone belongs to the [[transformer-enc-dec-tts]] family (FastSpeech 2), contrasting architecturally with the [[autoregressive-codec-tts]] approach exemplified by VALL-E ([[2301.02111|VALL-E]]), which achieves zero-shot quality at the cost of temporal latency. The companion paper [[2512.00937]] on Arabic TTS with FastPitch addresses spectral over-smoothing in the non-autoregressive continuous setting, the same degradation this paper targets through discrete classification.
